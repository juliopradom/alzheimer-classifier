...
def get_coefficients(
    ...
    """ Extracts Wavelet coefficients for a specific (or all) slices from the approx image
        ...
        Args:
            ...
            save (bool): True if results must be saved
        Returns:
            np-array: array of tuples. Each tuple contains the coefficients of one patient 
    ...
    for patient_class in INDEXES:
        ...
        for i in range(values["first"], values["last"]):
            ...
            try:
                ...
                if patient_slice:
                    coeffs = new_patient.get_slice_wavelet_coefficients(
                        wavelet_name=WAVELET_NAME,                                                
                        level=wavelet_level, 
                        slice_number=patient_slice
                    )
                else:
                    coeffs = new_patient.get_wavelet_coefficients(
                        wavelet_name=WAVELET_NAME, 
                        level=wavelet_level
                    )
...
def main(
    ...
    get_coefficients(patient_slice, 
                      wavelet_level, 
                      result_file,
                      save
                      )
